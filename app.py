...
# Mapping of ward numbers to namesimport streamlit as st
import pandas as pd
import plotly.express as px
import seaborn as sns
import matplotlib.pyplot as plt
...
# Fill missing values with defaults (for demonstration purposes)
...
# Data page
    ...
    if selected_ward:
        ...
        # Enhanced heatmap for water usage
        heatmap_data = ward_data.pivot_table(values='Monthly Water Usage (Liters)', index='Household ID', columns='Date', fill_value=0)
        fig5, ax5 = plt.subplots(figsize=(12, 8))
        sns.heatmap(
            heatmap_data, 
            ax=ax5, 
            cmap='coolwarm',        # Visually appealing color scheme
            linewidths=0.5,         # Add gridlines
            linecolor='gray',       # Light gridlines
            cbar_kws={'label': 'Water Usage (Liters)'},  # Color bar label
            annot=False,            # Set True for showing numbers
            fmt='g'
        )

        # Set title and labels
        ax5.set_title(f'Water Usage Distribution for Ward {selected_ward}', fontsize=16)
        ax5.set_xlabel('Date', fontsize=14)
        ax5.set_ylabel('Household ID', fontsize=14)
